...
* `amd`&ast; - включает узлы [n1-n4]. На них установлены процессоры AMD Epyc. Знак &ast; означает, если эта очередь используется по умолчанию, если не указана другая. На каждом узле доступно по 64 вычислительных ядра. Звездочка не является частью названия очереди. 
...
* `gpu` - включает только master-узел, на котором установлены две видеокарты. Старайтесь не задействовать CPU на 100%, так как этот узел также используется для управления очередью задач.
