...
На кластере параллельно работают 4 очереди задач:

* `amd*` - включает узлы [n1-n4]. На них установлены процессоры AMD Epyc. Знак `*` означает что эта очередь используется по умолчанию, если не указана другая. На каждом узле доступно по 64 вычислительных ядра. Звездочка не является частью названия очереди. 
* `intel` - включает узлы [n5-n12]. На них установлены процессоры Intel Xeon. На каждом узле доступно по 52 вычислительных ядра, работающих в 2 потока (Hyper-Threading). То есть, на одном узле доступно 104 ядра. Но в двухпоточном режиме скорость работы ядра падает вдвое. Если очереди задач явно задать условие использовать 52 ядра на узел, то система автоматически отключит Hyper-Threading для всех ядер, скорость работы каждого ядра не будет делиться на 2. Многопоточный режим дает выигрыш для задач, где интенсивно задействован обмен данными (c DDR либо Infiniband), и этот обмен является "узким" местом.
* `common` - включает все узлы [n1-n12]. Очереди `common`, `intel`, `amd*` не имеют между собой приоритета, задачи между ними распределяются в порядке живой очереди.
* `gpu` - включает только master-узел, на котором установлены две видеокарты. Старайтесь не задействовать CPU на 100%, так как этот узел также используется для управления очередью задач. Обычно gpu-задачи минимально задействуют CPU.

Используйте очереди `amd`,`intel` если ваши задачи жестко привязаны к числу ядер, либо требуют гомогенности от вычислительных узлов.
Используйте очередь `common` если ваша задача не привязана к характеристикам оборудования, либо требует 1-2 ядра процессора.
