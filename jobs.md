...
* `amd*` - включает узлы [n1-n4]. На них установлены процессоры AMD Epyc. Знак `*` означает, если эта очередь используется по умолчанию, если не указана другая. На каждом узле доступно по 64 вычислительных ядра. Звездочка не является частью названия очереди. 
